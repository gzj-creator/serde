...
- 补齐 mcpp 的公开头文件搜索路径和 `include/serde` 源码链接，使外部消费者
  能直接使用反射注册宏和内置 simdjson；新增独立 mcpp 消费工程验证结构体
  JSON/TOML 往返与缺失字段错误。
- CMake 的头文件和模块 target 不再强制向消费方传递 `-fno-exceptions`，
  模块采用所在工程的异常编译策略，simdjson 后端仍使用无异常接口；导出
  后端的标准头文件根路径，支持与其他库共用同一份实现。

### 文档

- 说明外部依赖的头文件路径、异常编译契约和 mcpp 消费回归测试入口。

### 维护

- 将 `CMakeLists.txt`、`MODULE.bazel` 与 `mcpp.toml` 的包版本声明同步更新为 `0.2.2`。
